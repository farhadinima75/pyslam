...
from kornia.feature import MKDDescriptor
# interface for pySLAM
class MKDDescriptorFeature2D: 
    def __init__(self, do_cuda=True):    
        print('Using MKDDescriptorFeature2D') 
        ...
        self.model = MKDDescriptor(patch_size=32,
                                    kernel_type='concat',
                                    whitening='pcawt',
    ...
    def compute(self, img, kps, mask=None):  #mask is a fake input  
        ...
        if kVerbose:
            print('descriptor: MKDDescriptor, #features: ', len(kps), ', frame res: ', img.shape[0:2])                  
        return kps, des
